        ...
        def sort_datetime_strings(datetime_obj):
            ...
            for dt in datetime_obj:
                if 'T' in dt['dateTime'] and ('+' in dt['dateTime'].split('T')[-1] or '-' in dt['dateTime'].split('T')[-1]):
                    ...
                    dt['dateTime'] = datetime.datetime.strptime(dt['dateTime'], '%Y-%m-%d')
                    dt['dateTime'] = dt['dateTime'].replace(tzinfo=datetime.timezone.utc)  # Add a default timezone (UTC) for naive datetimes
            ...
            sorted_datetime_obj = ['{0}  {1}'.format(dt['dateTime'].strftime('%Y-%m-%dT%H:%M:%S%z'), dt['summary']) for dt in sorted_datetime_objects]
...
async def auth(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    creds = update_token_crediential(update.effective_chat.id, token, force_update=True)
    if creds is None:
