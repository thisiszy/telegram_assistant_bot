    ...
    def print_uzh_menus(self, target_date, condition=None):
        ...
        for mensa in self.uzh_mensa_list:
            ...
            if self.args.lang == 'en':
                uzh_url = 'http://zfv.ch/de/menus/rssMenuPlan?menuId={}&dayOfWeek={}'.format(
                    mensa['idSlugEn'], str(weekday))
            elif self.args.lang == 'de':
                uzh_url = 'http://zfv.ch/de/menus/rssMenuPlan?menuId={}&dayOfWeek={}'.format(
                    mensa['idSlugDe'], str(weekday))
            else:
                    ...
                    for child in document[4][3][0]:
                        if (i % 2 == 0):
                            ...
                            formatted_text += "`{title} | {price}`\n".format(
                                title=cleanTitle, price=cleanPrice)
                        else:
...
class eth_mensa:
    ...
    def request_eth_facilities_list(self, rs_size=50, rs_first=0, lang="en"):
        eth_url = self.meta_link.format(
            lang=lang, rs_first=rs_first, rs_size=rs_size)

        context = ssl.create_default_context(cafile=certifi.where())
        with urllib.request.urlopen(eth_url, context=context) as url:
            data = json.loads(url.read().decode())['facility-array']
        # if rs_size equals len(data), then there are more facilities to be requested
        assert len(data) < rs_size
    ...
    def print_eth_menus(self, target_date, condition):
        ...
        def request_weekly_menus(rs_first, rs_size, valid_after):
            eth_url = self.data_link.format(
                lang=self.args.lang, rs_first=rs_first, rs_size=rs_size, valid_after=valid_after)
            context = ssl.create_default_context(cafile=certifi.where())
            with urllib.request.urlopen(eth_url, context=context) as url:
        ...
        eth_menu_data = request_weekly_menus(
            0, 50, target_date.strftime("%Y-%m-%d"))

        valid_data = [i for i in eth_menu_data if (datetime.datetime.strptime(i['valid-from'], "%Y-%m-%d").date() <= target_date and (
            'valid-to' not in i or datetime.datetime.strptime(i['valid-to'], "%Y-%m-%d").date() >= target_date))]
                    ...
                    if opening_hours['meal-time-array'] != []:
                        ...
                        for opening_hour in opening_hours['meal-time-array']:
                            formatted_text += "__{name}__\t{time_from}-{time_to}\n".format(
                                name=opening_hour['name'], time_from=opening_hour['time-from'], time_to=opening_hour['time-to'])
                            if 'line-array' in opening_hour:
                                for meal in opening_hour['line-array']:
                                    if 'meal' in meal:
                                        cur_menu = {}
                                        formatted_text += "`{name} | CHF {price}`\n".format(name=meal['meal']['name'], price='/'.join(
...
class MensaHandler(Handler):
    ...
    def info(self):
        return {
            ...
            "commands": [
                {"command": "mensa",
                    "description": r"Use /mensa <location\> command to check todays menu"}
            ]
        }
