...
def update_token_crediential(
    ...
    if not creds or not creds.valid:
        ...
        if secret is None:
            ...
            return None
        c.execute(
            "INSERT OR REPLACE INTO tokens (user_id, token, credentials) VALUES (?, ?, ?)",
...
def list_events(conn: sqlite3.Connection, c: sqlite3.Cursor, user_id: int) -> str:
    """Shows basic usage of the Google Calendar API.
    Prints the start and name of the next 10 events on the user's calendar.
    """
    try:
        creds = update_token_crediential(conn, c, user_id, None)
    except exceptions.RefreshError:
        c.execute("SELECT token FROM tokens WHERE user_id = ?", (user_id,))
        result = c.fetchone()
        if result:
            creds = update_token_crediential(
                conn, c, user_id, result[0], force_update=True
            )
        else:
    ...
    try:
        ...
        for calendar in list_calendar(service):
            ...
            for event in events:
                ...
                event_list.append({"dateTime": start, "summary": event["summary"]})

        def sort_datetime_strings(datetime_obj):
            # Convert datetime strings to datetime objects
            for dt in datetime_obj:
                if "T" in dt["dateTime"] and (
                    "+" in dt["dateTime"].split("T")[-1]
                    or "-" in dt["dateTime"].split("T")[-1]
                ):
                    dt["dateTime"] = datetime.datetime.fromisoformat(dt["dateTime"])
                else:
            ...
            sorted_datetime_objects = sorted(datetime_obj, key=lambda x: x["dateTime"])

            # Convert sorted datetime objects back to datetime strings
            sorted_datetime_obj = [
                "{0}  {1}".format(
                    dt["dateTime"].strftime("%Y-%m-%dT%H:%M:%S%z"), dt["summary"]
                )
                for dt in sorted_datetime_objects
            ]
...
class Calendar(Handler):
    ...
    async def events(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        result = list_events(self.conn, self.c, update.effective_chat.id)
    ...
    async def auth(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        creds = update_token_crediential(
            self.conn, self.c, update.effective_chat.id, token, force_update=True
        )
        if creds is None:
            await context.bot.send_message(
                chat_id=update.effective_chat.id, text="Token invalid"
            )
        else:
            await context.bot.send_message(
                chat_id=update.effective_chat.id, text="Token saved"
            )
    ...
    async def delete(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if event_id == "":
            await context.bot.send_message(
                chat_id=update.effective_chat.id, text="Please specify the event id"
            )
            return

        try:
            creds = update_token_crediential(self.conn, self.c, user_id, None)
        except exceptions.RefreshError:
            self.c.execute("SELECT token FROM tokens WHERE user_id = ?", (user_id,))
            result = self.c.fetchone()
            if result:
                creds = update_token_crediential(
                    self.conn, self.c, user_id, result[0], force_update=True
                )
            else:
                raise exceptions.RefreshError
        if creds is None:
